...
| Code style      | 🟢 **TypeScript** (`strict` + `noImplicitAny`), classes with `#private`, data objects; 15 Vitest specs. `npm run check` = tsc + eslint (`typescript-eslint` `recommendedTypeChecked`, undiluted) + test. TS pinned to `~6.0.3` until `typescript-eslint` supports the TS 7.1 API. |
